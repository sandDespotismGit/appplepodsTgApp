...
async function getTrack(trackNum) {
  ...
  console.log(track);
  let states = document.getElementsByClassName("track_info_state");
  let header = document.getElementsByClassName("track_info_header");
  if (track.status == "ok") {
    header[0].innerText = `Заказ №${trackNum}`;
    states[0].innerText = `${track.data.destinationPostalAddress}`;
    states[1].innerText = `${
      track.data.events[track.data.events.length - 1]
        .operationAttributeTranslated
    }`;
  } else {
    header[0].innerText = "ОШИБКА!";
  }
}
